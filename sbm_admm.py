...
def score(Y, num_groups=2, block_size=500, predicted_groups=None):
    ...
    return adjusted_rand_score(predicted_groups[:len(actual_labels)], actual_labels)
...
def admm_step(A, Y, Z, lam, beta, gamma, little_lambda):
    """
    A: adjacency matrix
...
def sdp_1_step(A, Y, Z, U, V, gamma, beta, rho, epsilon=0):
    """see Appendix J of the Amini & Levina paper..."""

    matrix_to_project = 1/(2 + epsilon) * (Z - U + Y - V + A / rho)
    X = sdp_1_projection(matrix_to_project)
    Z = np.maximum(0, 1/(1 - epsilon) * (X + U))
    Y = project_psd_cone(1/(1 - epsilon) * (X + V))
...
def batch_sdp_1(A, X=None, Y=None, Z=None, U=None, V=None, iters=4, score_func=score, verbose=True, gamma=0, beta=1, epsilon=0):
    ...
    for i in range(iters):
        X, Y, Z, U, V = sdp_1_step(A, Y, Z, U, V, gamma, beta, 1, epsilon)
